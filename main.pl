...
/*
purpose: find path from current position to target object
returns: a list of directions to follow to reach the object
*/
path_to(Object, Path) :-
    where(CurrentRoom, CurrentX, CurrentY),
    object_located(Object, TargetRoom, TargetX, TargetY, _),
    find_path(CurrentRoom, CurrentX, CurrentY, TargetRoom, TargetX, TargetY, 
             [[CurrentRoom,CurrentX,CurrentY]], [], RevPath),
    reverse(RevPath, Path).

/* Base case - reached destination in same room */
find_path(Room, X, Y, Room, X, Y, _, Path, Path).

/* Recursive case - move within current room */
find_path(Room1, X1, Y1, Room2, X2, Y2, Visited, CurrentPath, FinalPath) :-
    Room1 == Room2, % Same room
    direction(Direction, DX, DY),
    NewX is X1 + DX,
    NewY is Y1 + DY,
    room(Room1, RX, RY),
    NewX >= 0, NewX < RX,
    NewY >= 0, NewY < RY,
    \+ space_taken(Room1, NewX, NewY, _),
    \+ member([Room1,NewX,NewY], Visited),
    find_path(Room1, NewX, NewY, Room2, X2, Y2, 
             [[Room1,NewX,NewY]|Visited], [Direction|CurrentPath], FinalPath).

/* Recursive case - move through doorway to another room */
find_path(Room1, X1, Y1, Room2, X2, Y2, Visited, CurrentPath, FinalPath) :-
    Room1 \= Room2, % Different rooms
    doorway_exists(Room1, NextRoom, X1, Y1, DestX, DestY, Direction),
    \+ member([NextRoom,DestX,DestY], Visited),
    find_path(NextRoom, DestX, DestY, Room2, X2, Y2, 
             [[NextRoom,DestX,DestY]|Visited], [Direction|CurrentPath], FinalPath).
    
/*
purpose: navigate to an object
returns: prints the path to the object and follows it
*/
goto(Object) :-
    path_to(Object, Path),
